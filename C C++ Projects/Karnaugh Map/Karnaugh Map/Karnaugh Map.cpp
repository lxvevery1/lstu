...
int main()
{
	...
	GetFinalExpression();
	cout << "F(X, Y, Z) = " << FinalExpression << endl;
	system("pause");
}
		...
		for (int i = 0; i < ROWS; i++)
		{
			for (int j = 0; j < COMLUMNS; j++)
			{
				...
				if (Kmap[i][j].state == 1)
				{
					...
					{
						X.minterms.push_back(AllElemntsInImplicant[k]);
						cout << AllElemntsInImplicant[k].binaryminterm << " ";
					}
					cout << endl << "All elements in Implicant done." << endl;
					X.Size = AllElemntsInImplicant.size();
					AllImplicants.push_back(X);
					X.minterms.clear();
					X.Size = 0;
...
void GetFinalExpression()
{
	...
	for (int i = 0; i < AllImplicants.size(); i++)
	{
		...
		for (int L = 0; L < 3; L++)
		{
			...
			{
				cout << endl << "[m][L] == [k][L]" << MintermsInPI[m][L] << " " << MintermsInPI[k][L] << endl;
