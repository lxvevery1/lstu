...
int main() {
    ...
    while (x_point <= d) {
        ...
        for (int i = 0; i < 3; i++) { // вычисление приближенных значений производных
            y_diff_approx[i].push_back((lagrange_poly(x_point, xi, yi, i + 2) - lagrange_poly(x_point - step, xi, yi, i + 2)) / step);
            y_diff2_approx[i].push_back((lagrange_poly(x_point + step, xi, yi, i + 2) - 2 * lagrange_poly(x_point, xi, yi, i + 2) + lagrange_poly(x_point - step, xi, yi, i + 2)) / pow(step, 2));
        }
        x_point += h;
    }
    ...
    cout << setw(5) << "x" << setw(15) << "y" << setw(15) << "Exact y'" << setw(15) << "L2'" << setw(15) << "L3'" << setw(15) << "L4'" << setw(15) << "Exact y''" << setw(15) << "L2''" << setw(15) << "L3''" << setw(15) << "L4''\n\n";
